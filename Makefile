...
# Try to detect CUDA path, fallback to common locations
CUDA_PATH ?= /usr/local/cuda
ifneq ($(shell which nvcc 2>/dev/null),)
    CUDA_PATH = $(shell dirname $(shell dirname $(shell which nvcc)))
endif

# Override for CUDA 11.8 compatibility  
CUDA_PATH = /usr/local/cuda-11.8

CUDA_FLAGS = -std=c++14 -O3 -Wno-deprecated-gpu-targets --compiler-bindir /usr/bin/g++-9 -allow-unsupported-compiler --expt-relaxed-constexpr --expt-extended-lambda
CXX_FLAGS = -std=c++17 -O3 -Wall -Wextra -I$(CUDA_PATH)/include
...
gaml: $(CXX_OBJECTS) $(CUDA_OBJECTS)
	$(NVCC) $(CUDA_FLAGS) -arch=sm_75 -o $@ $^ $(CUDA_LIBS) -lpthread

# Proof of concept (original benchmark)
gaml_poc: cuda_q4k_dequant.cu
	$(NVCC) $(CUDA_FLAGS) -arch=sm_75 -o $@ $< $(CUDA_LIBS)

# CUDA object files
%.o: %.cu
	$(NVCC) $(CUDA_FLAGS) -arch=sm_75 -c $< -o $@
